            ...
            <div className="grid min-[1200px]:grid-cols-3 gap-12 xl:gap-16">
               ...
               <div className="min-[1200px]:max-w-md max-w-lg w-full">
                  ...
                  <div className="space-y-3 text-sm text-slate-400 mt-6">
                     <p className="leading-relaxed">Address: 123 Market Street, Suite 45, San Francisco</p>
                     <p>Phone: <a href="tel:+14155552671" className="hover:text-slate-50 transition-colors">+1 415 555 2671</a></p>
                     <p>Email: <a href="mailto:support@novaui.com" className="hover:text-slate-50 transition-colors">support@novaui.com</a></p>
                  </div>

                  <ul className="flex flex-wrap gap-6 mt-6">
                     {socialLinks.map((social) => (
                        <li key={social.label}>
                           <a href="#" aria-label={social.label} className="flex items-center bg-neutral-800 w-8 h-8 p-2 rounded-full hover:bg-neutral-700 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
               ...
               <div className="min-[1200px]:col-span-2 grid grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-6 sm:gap-x-8">
                  ...
                  <div className="space-y-6 max-lg:col-span-full max-w-md">
                     <div>
                        <h3 className="text-slate-50 text-sm font-semibold mb-6">Join Our Newsletter</h3>
                        <p className="text-slate-400 text-sm leading-relaxed">Subscribe to our newsletter and stay up to date with the latest news and updates.</p>
                     </div>

                     <form className="bg-neutral-800 flex p-1 focus-within:ring-2 focus-within:ring-blue-500 rounded-md">
                        <input type="email" placeholder="Enter your email" className="text-slate-50 text-sm w-full bg-transparent pl-2 outline-none" required />
                        <button type="submit" className="py-2 px-3.5 text-sm rounded-md font-semibold text-nowrap cursor-pointer tracking-wide text-white border border-blue-600 bg-blue-600 hover:bg-blue-700 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
                           Submit
                        </button>
                     </form>
                  </div>
            ...
            <div className="flex flex-wrap gap-4 flex-col md:flex-row items-center md:justify-between">
               <ul className="flex flex-wrap gap-4 text-sm text-slate-400 font-normal">
                  {["Terms of Service", "Privacy Policy", "Security"].map((legal) => (
                     <li key={legal}>
                        <a href="#" className="hover:text-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded transition-all">{legal}</a>
                     </li>
                  ))}
               </ul>
               <p className="text-slate-400 text-sm">©CASSI-STORE. All rights reserved.</p>
            </div>
         </div>
      </footer>
   );
